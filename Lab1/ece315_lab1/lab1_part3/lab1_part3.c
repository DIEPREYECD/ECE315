...
static void vButtonsTask(void *pvParameters) {
    ...
    while (1) {
        ...
        if (pushed == 8 || pushed == 1) {
            xQueueOverwrite(xQueueBTN_TO_RGB, &pushed);
            vTaskDelay(13);
            xil_printf("\nSent pushed: %d\n", pushed);
        }
    }
...
static void vRgbTask(void *pvParameters)
{
    ...
    while (1){
        if (xQueueReceive(xQueueBTN_TO_RGB, &pushed, 10) == pdTRUE) {
            vTaskDelay(16);
            xil_printf("\nReceived pushed: %d\n", pushed);
            if (pushed == 8) {
                // increase off
                if (xOffDelay + 1 < xDelay) {
                    xOffDelay += 1;
                    xOnDelay -= 1;
                }
...
static void vKeypadTask( void *pvParameters )
{
    ...
    /*************************** Enter your code here ****************************/
            ...
            */
            KYPD_DSP val = { .prev_key = previous_key, .cur_key = current_key };

            xQueueOverwrite(xQueueKYPD_TO_DSP, &val);
...
static void vDisplayTask(void *pvParameters) {
    ...
    while(1) {
        ...
        if (xQueueReceive(xQueueKYPD_TO_DSP, &val, 10) == pdTRUE) {
            cur_ssd_val = SSD_decode(val.cur_key, 1);
            prev_ssd_val = SSD_decode(val.prev_key, 0);
        }
